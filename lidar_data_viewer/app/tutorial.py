...
def read_map(filepath):
    d = {}
    with open(f'{filepath}.npy', 'rb') as f:
        keys = np.load(f)
        for key in keys:
            d[key] = np.load(f)
    return d
...
# Read Data
# d = read_map(os.path.join(os.path.dirname(os.path.dirname(CURRENT_DIRECTORY)), "data\dSomerville_s22000.npy"))

reader = vtkXMLUnstructuredGridReader()
reader.SetFileName(os.path.join(os.path.dirname(os.path.dirname(CURRENT_DIRECTORY)), "data\disk_out_ref.vtu"))
reader.Update()
...
# -----------------------------------------------------------------------------
# Model parameters ---------------------
@state.change("z_land_exaggeration")
def update_z_multiplier(z_land_exaggeration, **kwargs):
    z_land_exaggeration = z_land_exaggeration
    # ctrl.view_update()

@state.change("water_recess_layers")
def update_water_recess_layers(water_recess_layers, **kwargs):
    water_recess_layers = water_recess_layers
    # ctrl.view_update()

@state.change("road_emboss_layers")
def update_road_emboss(road_emboss_layers, **kwargs):
    road_emboss_layers = road_emboss_layers
    # ctrl.view_update()

@state.change("base_thickness")
def update_base_thickness(base_thickness, **kwargs):
    base_thickness = base_thickness
    print(clip_border)
    # ctrl.view_update()

@state.change("clip_border")
def update_border_clip(clip_border, **kwargs):
...
def mesh_card():
    with ui_card(title="Model Parameters"):
        ...
        vuetify.VTextField(
            label="Water Recess",
            v_model=("water_recess_layers", 10),
            suffix=("currentSuffix", ""),
        )
        vuetify.VTextField(
            label="Road Emboss",
            v_model=("road_emboss_layers", 10),
            suffix=("currentSuffix", ""),
        )
        vuetify.VTextField(
            label="Base Thickness",
            v_model=("base_thickness", 20),
            suffix=("currentSuffix", ""),
        )
        vuetify.VCheckbox(
            label="Clip with border",
            v_model=("clip_border", 20),
            dense=True,
        )


def contour_card():
    with ui_card(title="Display Settings"):
        vuetify.VSelect(
            # Contour By
            label="Contour by",
